import { z } from 'zod'
import { getCalendarEvents, createCalendarEvent, updateCalendarEvent, deleteCalendarEvent } from '@/app/actions/actions'
...
export const calendarTools = {
  get_calendar_events: {
    ...
    inputSchema: z.object({
      start: z.string().describe('Start date in ISO string format (e.g., "2025-08-22T00:00:00Z")'),
      end: z.string().describe('End date in ISO string format (e.g., "2025-08-22T23:59:59Z")'),
    }),
  ...
  update_calendar_event: {
    ...
    inputSchema: z.object({
      ...
      start: z.string().describe('New start date and time in ISO string format'),
      end: z.string().describe('New end date and time in ISO string format'),
      attendees: z.array(z.string()).optional().describe('New array of attendee email addresses'),
    }),
    execute: async ({ id, summary, description, start, end, attendees }: {
      id: string
      summary: string
      description?: string
      start: string
